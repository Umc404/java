...
public class File01 {
	...
	public static void main(String[] args) {
		// file : java.io.* 에서 제공하는 클래스들은 try~catch / throws를 해줘야 함.
		// Input/OutpubStream(IOStream) : 단방향으로 데이터가 흘러가는 형태.
		/* java 는 입력스트림, 출력스트림을 통해 데이터를 입출력함.
		 * 다양한 장치에 독립적으로 운영, 일관성있게 입출력을 유지하기 위해 사용.
		 * 입출력 스트림을 통해 일관성, 독립성 제공
		 * 
		 * 두가지 형태의 스트림
		 * - 바이트 형태의 스트림 : 기본.(문자, 그림, 멀티미디어) // 그림 멀티미디어는 문자형태 스트림 불가
		 * - 문자 형태의 스트림 : 문자만 입출력할 때 사용.
		 * 
		 * 1. 바이트 스트림
		 * - 입력(InputStream) : FileInputStream, BufferedInputStream, DataInputStream ...
		 * - 출력(OutputStream) : FileOutputStream, BufferedOutputStreaam, DataOutStream ...
		 * 
		...
		try {
			...
			System.out.println("Read() => "+ (char)b);	// 문자열 형변환을 하지 않으면 ascii code 번호 리턴.
		} catch(IOException e) {
			e.printStackTrace();
		}
