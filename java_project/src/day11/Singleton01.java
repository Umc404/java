...
public class Singleton01 {
	...
	public static void main(String[] args) {
		 ...
		 * 디자인 패턴 중 하나
		 * 싱글턴 : 단 1개만 존재해야하는 객체를 생성하여 => 공유하고자 할 때 사용
		 */
		
		// Singleton 객체 생성
//		Singleton s = new Singleton01();		// 안됨. 생성자가 private 선언.
		
		// static 메서드/멤버변수는 클래스명으로부터 바로 접근.
		Singleton s1 = Singleton.getInstance();
		Singleton s2 = Singleton.getInstance();
		System.out.println(s1);
		System.out.println(s2);					// s1, s2 두 주소가 같음. => 하나의 객체를 공유.
		
		Calendar c1 = Calendar.getInstance();	
		System.out.println(c1);
		Calendar c2 = Calendar.getInstance();	// 현 시점에서의 하나의 객체. c1, c2 값이 같음.
