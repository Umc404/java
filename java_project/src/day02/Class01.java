...
public class Class01 {
	...
	public static void main(String[] args) {
		 ...
		 * Lotto l = new Lotto(); // Lotto 클래스에서 객체를 생성 / 객체명 => 1
		 * 
		 * 클래스 : 객체를 생성하기 위한 설명서
		 * 객체 : 실제 사용되는 제품
		 * 메서드 : 기능
		 ...
		 * 
		 * - 클래스에 public을 붙일 수 있는 건 파일명과 클래스명이 같을 경우에만 가능.
		 * - .java 파일 하나에 하나의 클래스만 있는 건 아님. (여러개 가능)
		 * - 멤버변수와 메서드는 public 상관없음.
		 * - 일반적으로 각 클래스의 멤버변수는 private 이 원칙 / method는 public이 일반적
		 * - private은 내 클래스에서만 사용가능. (다른 클래스에서는 접근 불가능.)
		 * - private으로 선언된 멤버변수에 접근하기 위해서는 getter / setter 가 필요.
		 * 
		 * - 접근제한자
		 * private : 나(내 클래스)
		 * (default) : 나 + 패키지
...
class Test1{
	...
	// java 에서는 getter / setter 를 자동으로 생성 가능.
}
