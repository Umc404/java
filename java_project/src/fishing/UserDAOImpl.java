...
public class UserDAOImpl implements UserDAO {
	...
	public boolean compMoney(User u1) {
		System.out.println(u1.getCost() + "  dfdfdfdf  " + u1.getNum());			//	낚시대 가격
	...
	public List<Inventory> userInven(int userNum) {
		System.out.println("인벤토리 데이터 DAOImpl 확인 성공 " + userNum);
	...
	public List<Inventory> userRank(int num) {
		...
		try {
			...
			ResultSet rs = pst.executeQuery();
			while(rs.next()) {
//				list.add(new Inventory(rs.getInt("num"),
