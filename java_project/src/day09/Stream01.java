...
public class Stream01 {
	public static void main(String[] args) {
		 ...
		 *   forEach(), count(), sum(), allMatch() : 스프링요소가 전부 일치하는지
		 *   anyMatch() : 하나라도 일치하는지..
		 *   toArray() : 배열로 변환
		 *   collect() : 스트림 수집
		 */
		...
		System.out.println("## 스트림 활용 합계 ##");
		
		int total = Arrays.stream(arr).sum();	// sum() : int, long 둘 다 호환
		...
		int[] score = {78, 89, 56, 82, 43, 68, 70, 76, 88};
		
		// Stream 이용하여 합계 평균 최대값
		// 합계
		int sum3 = Arrays.stream(score).sum();
		// 평균
		OptionalDouble avg2 = Arrays.stream(score).average();
		
		System.out.println(Math.round(avg2.getAsDouble()*100)/100.0); // 수수점 둘째자리까지 반올림
		// 최대값
		OptionalInt max = Arrays
				.stream(score)
				.max();
		System.out.println(max.getAsInt());
		// Stream 이용하여 70 이상의 인원수 출력
		long cnt2 = Arrays
				.stream(score)
				.filter(n -> n>=70)
				.count();
...
//		int s = list.stream().sum();
		...
		System.out.println(s1);		// 주소값임.
		
		int s2 = s1.mapToInt(n->n.intValue()).sum();
		System.out.println(s2);
		
		// 이미 소모된 스트림은 다시 사용할 수 없음. 위에서 s2가 사용함.
//		OptionalDouble avg3 = 
//				s1.mapToInt(n->n.intValue())
//				.average();
